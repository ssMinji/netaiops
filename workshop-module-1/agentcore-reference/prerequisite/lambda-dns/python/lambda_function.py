...
def find_aws_resources_by_ip(ip_address: str, region: str = 'us-east-1') -> List[Dict[str, Any]]:
    """
    Find EC2 instances, ENIs, and RDS instances that have the specified private IP address.
    ...
    try:
        ...
        # Search RDS instances (they don't show up in EC2 describe calls)
        try:
            rds_response = rds_client.describe_db_instances()
            for db_instance in rds_response.get('DBInstances', []):
                db_endpoint = db_instance.get('Endpoint', {})
                if db_endpoint:
                    # Try to resolve the RDS endpoint to see if it matches our IP
                    rds_hostname = db_endpoint.get('Address', '')
                    if rds_hostname:
                        try:
                            rds_ip = socket.gethostbyname(rds_hostname)
                            if rds_ip == ip_address:
                                found_resources.append({
                                    'type': 'RDS Database Instance',
                                    'id': db_instance.get('DBInstanceIdentifier', 'N/A'),
                                    'name': db_instance.get('DBName', 'N/A'),
                                    'private_ip': ip_address,
                                    'state': db_instance.get('DBInstanceStatus', 'N/A'),
                                    ...
                                    'endpoint': rds_hostname
                                })
                                logger.info(f"Found RDS instance: {db_instance.get('DBInstanceIdentifier')} ({rds_hostname})")
                        except socket.gaierror:
                            # RDS hostname doesn't resolve, skip
                            continue
        except Exception as rds_error:
            logger.warning(f"Error searching RDS instances: {str(rds_error)}")
...
def handle_dns_resolve(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if found_resources:
            ...
            rds_instances = [r for r in found_resources if r['type'] == 'RDS Database Instance']
            ...
            if rds_instances:
                result_text += "🗄️ RDS Database Instances:\n"
                for i, resource in enumerate(rds_instances, 1):
                    result_text += f"  {i}. DB Instance ID: {resource['id']}\n"
                    result_text += f"     DB Name: {resource['name']}\n"
                    result_text += f"     Private IP: {resource['private_ip']}\n"
                    result_text += f"     State: {resource['state']}\n"
                    result_text += f"     Engine: {resource['engine']} {resource['engine_version']}\n"
                    result_text += f"     Instance Class: {resource['instance_class']}\n"
                    result_text += f"     VPC: {resource['vpc_id']}\n"
                    result_text += f"     Subnet Group: {resource['subnet_group']}\n"
                    result_text += f"     AZ: {resource['availability_zone']}\n"
                    result_text += f"     Endpoint: {resource['endpoint']}\n\n"
