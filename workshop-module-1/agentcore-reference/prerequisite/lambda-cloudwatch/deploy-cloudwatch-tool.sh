...
# CloudWatch Tools Lambda Deployment - Docker Container Approach
# Provides CloudWatch Alarms, Logs, and Metrics analysis capabilities
set -e

echo "📊 CloudWatch Tools Deployment"
echo "=================================="

# Configuration
REGION="us-east-1"
STACK_NAME="cloudwatch-tools-lambda"
ECR_REPOSITORY="cloudwatch-tools-repo"
...
# Build Docker image for CloudWatch tools (no size limits!)
echo "🐳 Building Docker image for CloudWatch Lambda (x86_64 architecture)..."
cd python
docker build --platform linux/amd64 -t cloudwatch-tools-lambda:latest .
cd ..
...
docker tag cloudwatch-tools-lambda:latest ${ECR_URI}:latest
docker push ${ECR_URI}:latest

# Deploy Lambda function directly with AWS CLI
echo "🚀 Deploying CloudWatch Tools Lambda function with AWS CLI..."

FUNCTION_NAME="cloudwatch-tools"
ROLE_NAME="cloudwatch-tools-role"
...
# Add CloudWatch permissions for alarms, logs, and metrics
aws iam put-role-policy \
    --role-name ${ROLE_NAME} \
    --policy-name CloudWatchToolsPolicy \
    --policy-document '{
        "Version": "2012-10-17",
        "Statement": [
            {
...
echo "⚡ Creating CloudWatch Tools Lambda function from container image..."
...
CLOUDWATCH_LAMBDA_ARN="arn:aws:lambda:${REGION}:${ACCOUNT_ID}:function:${FUNCTION_NAME}"
...
aws ssm put-parameter \
    --name "/app/troubleshooting/agentcore/cloudwatch_lambda_arn" \
    --value "$CLOUDWATCH_LAMBDA_ARN" \
    --type "String" \
    --overwrite \
    --region $REGION
...
echo ""
echo "✅ CloudWatch Tools Lambda Deployment Complete!"
echo "   CloudWatch Lambda ARN: $CLOUDWATCH_LAMBDA_ARN"
echo "   Gateway Role ARN: $GATEWAY_ROLE_ARN"
echo "   Container Image: ${ECR_URI}:latest"
echo ""
echo "📊 CloudWatch Tools Features:"
echo "   • CloudWatch Alarms: Create, describe, and manage alarms"
echo "   • CloudWatch Logs: Query and analyze log events"
echo "   • CloudWatch Metrics: Retrieve and analyze metrics data"
echo "   • Multi-service support: EC2, RDS, Lambda, and more"
echo ""
echo "🎯 Next Steps:"
echo "   1. Create gateway with CloudWatch tools:"
echo "      python ../../scripts/agentcore_gateway.py create --name troubleshooting-gateway"
echo "   2. Deploy runtime:"
echo "      python ../../scripts/agentcore_agent_runtime.py create --name troubleshooting_agent_runtime"
echo ""
echo "📝 Usage Examples:"
echo "   • 'Show me all critical alarms in the last hour'"
echo "   • 'Get error logs from /aws/lambda/my-function for the last 30 minutes'"
echo "   • 'What are the CPU metrics for instance i-1234567890abcdef0?'"
