from agent_config.context import K8sContext
...
async def invoke(payload, context):
    if not K8sContext.get_response_queue_ctx():
        K8sContext.set_response_queue_ctx(StreamingQueue())

    if not K8sContext.get_gateway_token_ctx():
        K8sContext.set_gateway_token_ctx(await get_gateway_access_token())
