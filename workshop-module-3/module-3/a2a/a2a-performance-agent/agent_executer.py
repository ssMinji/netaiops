...
class PerformanceAgentCoreExecutor(AgentExecutor):
    """
    A2A executor that fronts a Bedrock AgentCore Runtime (Performance agent)
    ...
    def _invoke_json(self, payload: Dict[str, Any], headers: Dict[str, str]) -> str:
        ...
        if response_text.startswith('data:') or 'text/event-stream' in content_type:
            logger.debug("Detected SSE format response, parsing...")
            accumulated_content = []
            ...
            for line in response_text.split('\n'):
                ...
                if line.startswith('data:'):
                    ...
                    # Skip empty data lines or special markers
                    if not data_content or data_content in ['#', '[DONE]']:
                        continue
                    ...
                    try:
                        ...
                        # Check if data_json is a dict before calling .get()
                        if isinstance(data_json, dict):
                            text_content = (
                                data_json.get('text') or
                                data_json.get('content') or
                                data_json.get('output') or
                                data_json.get('response') or
                                data_json.get('message') or
                                str(data_json)
                            )
                        else:
                            # If it's a string or other type, use it directly
                            text_content = str(data_json)
                        accumulated_content.append(text_content)
                    except json.JSONDecodeError:
                        # If not JSON, use the raw data content
                        accumulated_content.append(data_content)
            
            if accumulated_content:
                result = ' '.join(accumulated_content)
                logger.debug(f"Parsed SSE content: {result[:200]}...")
                return result
            else:
                logger.warning("No valid content found in SSE response")
        ...
        try:
            ...
            logger.debug(f"Response is not JSON (error: {e}), treating as plain text")
    ...
    async def execute(
        ...
        try:
            ...
            await updater.add_artifact(parts, name="performance_analysis_result")
            ...
            logger.exception("PerformanceAgentCoreExecutor failed")
