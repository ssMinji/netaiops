...
# A2A Performance Agent - ECS Cluster and Service Deployment (Part 1/3)
...
log_info "Starting A2A Performance Agent ECS Cluster and Service Deployment (Part 1/3)"
...
for attempt in $(seq 1 $MAX_RETRIES); do
    ...
    if [ "$SERVICE_STATUS" = "ACTIVE" ]; then
        log_warning "Service a2a-performance-agent-service already exists and is ACTIVE. Updating with new task definition..."
...
#   aws ecs describe-services --cluster $CLUSTER_NAME --services a2a-performance-agent-service
# And check task logs in CloudWatch Logs: /ecs/a2a-performance-agent
...
TASK_ARN=$(aws ecs list-tasks --cluster $CLUSTER_NAME --service-name a2a-performance-agent-service --query "taskArns[0]" --output text)
if [ "$TASK_ARN" != "None" ] && [ -n "$TASK_ARN" ]; then
    ...
    if [ -n "$TASK_PUBLIC_IP" ] && [ "$TASK_PUBLIC_IP" != "None" ]; then
        ...
        log_info "You can test the service at: http://$TASK_PUBLIC_IP:10005/health"
        
        # Save task IP to config for reference
        jq --arg task_ip "$TASK_PUBLIC_IP" '.agentcore_performance.task_ip = $task_ip' "$STAGE4_CONFIG" > "$STAGE4_CONFIG.tmp" && mv "$STAGE4_CONFIG.tmp" "$STAGE4_CONFIG"
    fi
fi
...
log_success "A2A Performance Agent ECS Cluster and Service deployment completed!"
log_info "Cluster: $CLUSTER_NAME"
log_info "Service: a2a-performance-agent-service"
