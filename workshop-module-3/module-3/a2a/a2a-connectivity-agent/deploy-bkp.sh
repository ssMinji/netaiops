...
# A2A Connectivity Agent - Parent Deployment Script
...
# Start deployment
log_milestone "Starting A2A Connectivity Agent Parent Deployment"
...
log_milestone "STEP 2/3: Deploying A2A Connectivity Agent to ECS"
if run_script "deploy-to-ecs.sh" "ECS Deployment with Application Load Balancer"; then
    log_milestone "✓ A2A Connectivity Agent deployed to ECS successfully"
...
log_success "✓ A2A Connectivity Agent deployed to ECS"
...
if [ -f "deploy-to-ecs.log" ]; then
    ...
    log_info "ECS Service: a2a-connectivity-agent-service"
fi
...
log_info "2. Check CloudWatch logs: /ecs/a2a-connectivity-agent"
log_info "3. Monitor the ECS service in the AWS Console"
log_info "4. The agent is now ready for A2A connectivity troubleshooting"
echo ""
