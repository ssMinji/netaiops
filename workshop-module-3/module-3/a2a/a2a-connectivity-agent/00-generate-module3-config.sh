...
# generate-module3-config.sh
# Generates module3-config.json from SSM parameters and AWS resources
...
echo "🔧 Generating module3-config.json..."
echo "📍 Region: $REGION"

# Get troubleshooting agent ARN
echo "🔍 Fetching troubleshooting agent ARN..."
RUNTIME_ARN=$(aws bedrock-agentcore-control list-agent-runtimes \
    --region "$REGION" \
    --query 'agentRuntimes[?agentRuntimeName==`a2a_troubleshooting_agent_runtime`].agentRuntimeArn' \
    --output text)

if [ -z "$RUNTIME_ARN" ]; then
    echo "❌ Error: a2a_troubleshooting_agent_runtime not found"
    exit 1
fi
