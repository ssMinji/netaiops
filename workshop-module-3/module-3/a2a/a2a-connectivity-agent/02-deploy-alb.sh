...
# A2A Connectivity Troubleshooting Agent - Application Load Balancer Deployment (Part 2/3)
...
log_info "Starting A2A Connectivity Troubleshooting Agent ALB Deployment (Part 2/3)"
...
jq --arg subnet1 "$SUBNET_1" --arg subnet2 "$SUBNET_2" \
   '.connectivity_alb_subnets = {subnet_1: $subnet1, subnet_2: $subnet2}' \
...
jq --arg alb_sg_id "$ALB_SG_ID" '.connectivity_alb_security_group_id = $alb_sg_id' \
   "$STAGE4_CONFIG" > "$STAGE4_CONFIG.tmp" && mv "$STAGE4_CONFIG.tmp" "$STAGE4_CONFIG"
...
cat > update_connectivity_alb_config.py << 'EOF'
#!/usr/bin/env python3
import json
import sys
import os

def update_connectivity_alb_config():
    ...
    try:
        ...
        config['agentcore_troubleshooting']['alb_arn'] = alb_arn
        config['agentcore_troubleshooting']['alb_dns'] = alb_dns
...
if __name__ == "__main__":
    update_connectivity_alb_config()
EOF
...
if python3 update_connectivity_alb_config.py; then
    log_success "ALB information saved to module3-config.json"
    rm update_connectivity_alb_config.py
else
...
jq --arg target_group_arn "$TARGET_GROUP_ARN" \
   '.agentcore_troubleshooting.target_group_arn = $target_group_arn' \
   "$STAGE4_CONFIG" > "$STAGE4_CONFIG.tmp" && mv "$STAGE4_CONFIG.tmp" "$STAGE4_CONFIG"
...
log_success "A2A Connectivity Troubleshooting Agent ALB deployment completed successfully!"
