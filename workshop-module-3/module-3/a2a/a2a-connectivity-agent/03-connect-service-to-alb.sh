...
# A2A Connectivity Troubleshooting Agent - Connect Service to ALB (Part 3/3)
...
log_info "Starting A2A Connectivity Troubleshooting Agent Service-to-ALB Connection (Part 3/3)"
...
ALB_SG_ID=$(jq -r '.connectivity_alb_security_group_id // empty' "$STAGE4_CONFIG")
TARGET_GROUP_ARN=$(jq -r '.agentcore_troubleshooting.target_group_arn // empty' "$STAGE4_CONFIG")
ALB_DNS_NAME=$(jq -r '.agentcore_troubleshooting.alb_dns // empty' "$STAGE4_CONFIG")
...
# Get ALB subnets and their AZs (using connectivity-specific config paths)
ALB_SUBNET_1=$(jq -r '.connectivity_alb_subnets.subnet_1 // .alb_subnets.subnet_1 // empty' "$STAGE4_CONFIG")
ALB_SUBNET_2=$(jq -r '.connectivity_alb_subnets.subnet_2 // .alb_subnets.subnet_2 // empty' "$STAGE4_CONFIG")
...
if [ "$TASK_AZ" != "$ALB_AZ_1" ] && [ "$TASK_AZ" != "$ALB_AZ_2" ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        aws ecs wait services-stable --cluster $CLUSTER_NAME --services a2a-connectivity-agent-service --region $AWS_REGION
...
log_success "A2A Connectivity Troubleshooting Agent Service-to-ALB connection completed successfully!"
