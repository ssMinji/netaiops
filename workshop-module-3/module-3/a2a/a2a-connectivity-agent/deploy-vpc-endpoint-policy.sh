...
log_info "Checking VPC endpoint state..."
...
while [ $ELAPSED -lt $MAX_WAIT_TIME ]; do
    ...
    if [ "$ENDPOINT_STATE" == "available" ]; then
        log_success "VPC endpoint is available and ready for modification"
        break
    elif [ "$ENDPOINT_STATE" == "pending" ] || [ "$ENDPOINT_STATE" == "pendingAcceptance" ]; then
        log_info "VPC endpoint is in '$ENDPOINT_STATE' state, waiting... (${ELAPSED}s elapsed)"
        sleep $WAIT_INTERVAL
        ELAPSED=$((ELAPSED + WAIT_INTERVAL))
    else
        log_warning "VPC endpoint is in unexpected state: $ENDPOINT_STATE"
        sleep $WAIT_INTERVAL
        ELAPSED=$((ELAPSED + WAIT_INTERVAL))
    fi
done
...
if [ $ELAPSED -ge $MAX_WAIT_TIME ]; then
    log_error "Timeout waiting for VPC endpoint to become available"
    log_error "Current state: $ENDPOINT_STATE"
    log_info "You can manually retry this script after the VPC endpoint operation completes"
    exit 1
fi
...
# Update the VPC endpoint policy with retry logic
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    if aws ec2 modify-vpc-endpoint \
        ...
        --policy-document file://bedrock-agentcore-vpc-endpoint-policy.json 2>&1; then
        ...
        if [ $RETRY_COUNT -lt $MAX_RETRIES ]; then
            log_warning "Update failed, retrying in 30 seconds... (attempt $RETRY_COUNT of $MAX_RETRIES)"
            sleep 30
        fi
    fi
done
...
if [ "$UPDATE_SUCCESS" != "true" ]; then
    log_error "Failed to update VPC endpoint policy after $MAX_RETRIES attempts"
    log_error "The VPC endpoint may still be processing a previous operation"
    log_info "Wait a few minutes and retry the deployment with: CONTINUE_ON_FAILURE=true ./deploy-preprovision.sh"
    exit 1
fi
...
log_info "You can now retry running your a2a-connectivity-agent"
