...
# A2A Connectivity Troubleshooting Agent - ECS Deployment Script
# This script automates the complete deployment of the A2A Connectivity Troubleshooting Agent to AWS ECS
...
log_info "Starting A2A Connectivity Troubleshooting Agent ECS Deployment"
...
# Connectivity Agent Task Definition
  ...
  "containerDefinitions": [
    {
      ...
      "healthCheck": {
        "command": [
          "CMD-SHELL",
          "python3 -c \"import urllib.request; urllib.request.urlopen('http://localhost:10003/health', timeout=10)\" || exit 1"
        ],
        "interval": 30,
        "timeout": 10,
        "retries": 3,
...
ALB_SG_ID=$(aws ec2 create-security-group \
  --group-name a2a-connectivity-alb-sg \
  --description "Security group for A2A Connectivity ALB" \
  --vpc-id $VPC_ID \
  --query "GroupId" --output text 2>/dev/null || \
  aws ec2 describe-security-groups --filters "Name=group-name,Values=a2a-connectivity-alb-sg" --query "SecurityGroups[0].GroupId" --output text)
...
ALB_ARN=$(aws elbv2 create-load-balancer \
  ...
  aws elbv2 describe-load-balancers --names a2a-connectivity-alb --query "LoadBalancers[0].LoadBalancerArn" --output text)

log_info "ALB created: $ALB_ARN"

# Configure ALB idle timeout to 5 minutes (300 seconds)
log_info "Configuring ALB idle timeout to 5 minutes (300 seconds)..."
...
if [ $? -eq 0 ]; then
    log_success "ALB idle timeout configured to 300 seconds (5 minutes)"
else
...
cat > update_connectivity_dns.py << 'EOF'
#!/usr/bin/env python3
import json
import sys
import os

def update_connectivity_dns():
    # Get the DNS value from environment variable
    connectivity_dns = os.environ.get('ALB_DNS_NAME')
    
    if not connectivity_dns:
    ...
    try:
        ...
        # Ensure agentcore_troubleshooting section exists
        if 'agentcore_troubleshooting' not in config:
            config['agentcore_troubleshooting'] = {}
        
        # Add the alb_dns value
        config['agentcore_troubleshooting']['alb_dns'] = connectivity_dns
        
        # Write updated config
        with open(config_file, 'w') as f:
            json.dump(config, f, indent=2)
        
        print(f"Successfully updated module3-config.json with alb_dns: {connectivity_dns}")
...
TARGET_GROUP_ARN=$(aws elbv2 create-target-group \
  ...
  aws elbv2 describe-target-groups --names a2a-connectivity-tg --query "TargetGroups[0].TargetGroupArn" --output text)
...
# Step 12: Force Delete and Recreate ECS Service
log_info "Step 12: Force deleting and recreating ECS service..."
...
EXISTING_SERVICE=$(aws ecs describe-services --cluster $CLUSTER_NAME --services a2a-connectivity-agent-service --region $AWS_REGION --query "services[0].serviceArn" --output text 2>/dev/null)

if [ "$EXISTING_SERVICE" != "None" ] && [ "$EXISTING_SERVICE" != "" ] && [ "$EXISTING_SERVICE" != "null" ]; then
    log_warning "Service a2a-connectivity-agent-service already exists: $EXISTING_SERVICE"
    log_info "Forcefully deleting existing service..."
    
    # Set desired count to 0 first to stop tasks
    ...
    while [ $elapsed_time -lt $max_wait ]; do
        ...
        if [ "$service_status" = "INACTIVE" ] || [ "$service_status" = "None" ] || [ -z "$service_status" ]; then
            log_success "Service successfully deleted"
            break
        fi
        
        log_info "Service status: $service_status (waited ${elapsed_time}s)"
        sleep $check_interval
        elapsed_time=$((elapsed_time + check_interval))
    done
    
    if [ $elapsed_time -ge $max_wait ]; then
        log_error "Service deletion did not complete within ${max_wait} seconds"
        log_error "Proceeding anyway, but service creation may fail"
    fi
    
    # Additional wait to ensure AWS has fully cleaned up
    log_info "Waiting additional 15 seconds for AWS cleanup..."
    sleep 15
fi
...
EOF
    ...
    log_info "Network configuration file created at /tmp/network-config.json"
    log_info "Contents:"
    cat /tmp/network-config.json
    
    # Disable exit on error temporarily to capture the full error message
    set +e
    
    # Create service with proper JSON configuration
    log_info "Executing aws ecs create-service command..."
    SERVICE_CREATE_OUTPUT=$(aws ecs create-service \
      --cluster $CLUSTER_NAME \
    ...
    # Extract service ARN from output if successful
    if [ $SERVICE_CREATE_EXIT_CODE -eq 0 ]; then
        SERVICE_ARN=$(echo "$SERVICE_CREATE_OUTPUT" | jq -r '.service.serviceArn // empty' 2>/dev/null)
        if [ -z "$SERVICE_ARN" ]; then
            log_error "Service creation command succeeded but could not extract service ARN"
            log_error "Full output:"
            echo "$SERVICE_CREATE_OUTPUT"
            exit 1
        fi
    else
        SERVICE_ARN=""
    ...
    if [ $SERVICE_CREATE_EXIT_CODE -ne 0 ] || [ -z "$SERVICE_ARN" ]; then
        log_error "Failed to create ECS service (exit code: $SERVICE_CREATE_EXIT_CODE)"
...
if wait_for_service_stable_and_healthy "$CLUSTER_NAME" "a2a-connectivity-agent-service" "$TARGET_GROUP_ARN"; then
    log_success "A2A Connectivity Troubleshooting Agent service is running and healthy!"
    
    # Display final deployment information
    log_info "=== DEPLOYMENT SUMMARY ==="
    log_info "Service Name: a2a-connectivity-agent-service"
    ...
    test_health_endpoint_comprehensive() {
        ...
        log_info "Check CloudWatch logs: /ecs/a2a-connectivity-agent"
        return 1
    }
    ...
    log_error "CloudWatch log group: /ecs/a2a-connectivity-agent"
    exit 1
fi
...
log_success "A2A Connectivity Troubleshooting Agent deployment completed successfully!"
