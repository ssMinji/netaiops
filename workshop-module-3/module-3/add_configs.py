...
Usage:
    python3 add_cognito_scopes.py <stack_name> <region> <account_id> <runtime_id> [config_file_path]
    
Example:
    python3 add_cognito_scopes.py troubleshooting-agentcore-cognito us-east-1 237616366264 a2a_troubleshooting_agent_runtime-si1KrHHRxl
    python3 add_cognito_scopes.py troubleshooting-agentcore-cognito us-east-1 237616366264 a2a_troubleshooting_agent_runtime-si1KrHHRxl /path/to/module3-config.json
"""
...
def construct_runtime_arn(region, account_id, runtime_id):
    """Construct the runtime ARN from runtime_id, region, and account_id."""
    # Extract just the runtime identifier part (after the last /)
    # If runtime_id is already just the ID part, use it as is
    # If it's a full ARN, extract the ID part
    if runtime_id.startswith('arn:'):
        # Extract the runtime ID from the ARN
        runtime_id_part = runtime_id.split('/')[-1]
    else:
        runtime_id_part = runtime_id
    
    # Construct the ARN with the provided account_id
    runtime_arn = f"arn:aws:bedrock-agentcore:{region}:{account_id}:runtime/{runtime_id_part}"
    
    print(f"Constructed runtime ARN:")
    print(f"  Region: {region}")
    print(f"  Account ID: {account_id}")
    print(f"  Runtime ID: {runtime_id_part}")
    print(f"  Full ARN: {runtime_arn}")
    
    return runtime_arn

def update_config_file(config_file_path, machine_client_id, cognito_provider, cognito_auth_scope, cognito_discovery_url, runtime_arn=None):
    """Update the module3-config.json file with Cognito values and runtime ARN."""
    try:
        ...
        print(f"\nDetermining config section from cognito_provider: {cognito_provider}")
        ...
        if 'troubleshooting' in cognito_provider.lower():
            ...
            print(f"Warning: Could not determine section from cognito_provider, defaulting to agentcore_troubleshooting")
            section_key = 'agentcore_troubleshooting'
        
        print(f"Selected section: {section_key}")
        ...
        # Update runtime_arn if provided
        if runtime_arn:
            print(f"Updating runtime_arn in {section_key}: {runtime_arn}")
            config[section_key]['runtime_arn'] = runtime_arn
        else:
            print(f"Warning: No runtime_arn provided to update in {section_key}")
        ...
        if runtime_arn:
            ...
            print(f"  runtime_arn: NOT UPDATED (was None)")
...
def main():
    ...
    if len(sys.argv) < 5 or len(sys.argv) > 6:
        ...
        print("\nUsage:")
        print("    python3 add_cognito_scopes.py <stack_name> <region> <account_id> <runtime_id> [config_file_path]")
        print("\nExample:")
        print("    python3 add_cognito_scopes.py troubleshooting-agentcore-cognito us-east-1 237616366264 a2a_troubleshooting_agent_runtime-si1KrHHRxl")
        print("    python3 add_cognito_scopes.py troubleshooting-agentcore-cognito us-east-1 237616366264 a2a_troubleshooting_agent_runtime-si1KrHHRxl /path/to/module3-config.json")
        sys.exit(1)
    ...
    if len(sys.argv) == 6:
        ...
        # Default to module3-config.json in the same directory as the script
    ...
    # Step 5: Get cognito_provider from SSM (needed for both runtime ARN and config update)
    print("\n" + "=" * 60)
    print("Step 5: Getting Cognito Provider from SSM")
    print("=" * 60)
    ...
    # Step 6: Construct Runtime ARN
    print("\n" + "=" * 60)
    print("Step 6: Constructing Runtime ARN")
    print("=" * 60)
    
    runtime_arn = construct_runtime_arn(region, account_id, runtime_id)
    print(f"\n✓ Successfully constructed runtime ARN: {runtime_arn}")
    
    # Step 7: Update config file
    print("\n" + "=" * 60)
    print("Step 7: Updating Configuration File")
    print("=" * 60)
    ...
    if not cognito_provider:
        ...
        try:
            for param_path in ['/app/troubleshooting/agentcore/cognito_discovery_url', '/a2a/app/performance/agentcore/cognito_discovery_url']:
                try:
                    ...
                    print(f"Found discovery_url in SSM: {discovery_url}")
                    break
                except ClientError:
                    continue
        except Exception:
        ...
        if not discovery_url:
            ...
            print(f"Using constructed discovery_url: {discovery_url}")
        
        print(f"\nPreparing to update config file with:")
        print(f"  config_file_path: {config_file_path}")
        print(f"  machine_client_id: {machine_client_id}")
        print(f"  cognito_provider: {cognito_provider}")
        print(f"  cognito_auth_scope: {new_scope_value}")
        print(f"  cognito_discovery_url: {discovery_url}")
        print(f"  runtime_arn: {runtime_arn if runtime_arn else 'None (will not be updated)'}")
    ...
    # Step 8: Retrieve and store machine client secret in SSM
    print("\n" + "=" * 60)
    print("Step 8: Retrieving and Storing Machine Client Secret")
    print("=" * 60)
    
    try:
        # Get the machine client secret from Cognito
        print(f"Retrieving client secret for machine client: {machine_client_id}")
        response = cognito_client.describe_user_pool_client(
            UserPoolId=user_pool_id,
        ...
        if client_secret:
            ...
            ssm_param_name = '/app/troubleshooting/agentcore/machine_client_secret'
            ...
            try:
                ssm_client.put_parameter(
                    ...
                    Value=client_secret,
                    Type='SecureString',
                    Description='Machine client secret for Cognito OAuth2',
                    Overwrite=True
                )
                print(f"✓ Successfully stored client secret in SSM: {ssm_param_name}")
            except ClientError as e:
                print(f"Error storing client secret in SSM: {e}")
                print(f"You may need to manually store the secret using:")
                print(f"  aws ssm put-parameter --name '{ssm_param_name}' --value '<secret>' --type SecureString --overwrite")
        else:
            print("Warning: Machine client does not have a secret (it may be a public client)")
            print("If this is unexpected, you may need to recreate the client with GenerateSecret=true")
            
    except ClientError as e:
        print(f"Error retrieving client secret: {e}")
        print("You may need to manually retrieve and store the secret")
