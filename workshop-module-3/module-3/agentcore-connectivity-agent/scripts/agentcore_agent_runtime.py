...
def create(name: str):
    ...
    1. Checks if runtime already exists (idempotency)
    ...
    # STEP 1: Check if runtime already exists (idempotency)
    click.echo(f"🔍 Checking if runtime '{name}' already exists...")
    try:
        agentcore_control_client = boto3.client("bedrock-agentcore-control", region_name=get_aws_region())
        
        # List all runtimes and search for matching name
        ...
        for runtime in runtimes_response.get("agentRuntimes", []):
            if runtime["agentRuntimeName"] == name:
                ...
                click.echo(f"✅ Runtime '{name}' already exists")
                click.echo(f"   Runtime ARN: {runtime_arn}")
                click.echo(f"   Runtime ID: {runtime_id}")
                click.echo(f"ℹ️  Skipping creation")
                
                # Update module3-config.json with existing runtime ARN
                click.echo("📝 Updating module3-config.json with existing runtime ARN...")
                try:
                    ...
                    if config_file.exists():
                        ...
                        config['agentcore_troubleshooting']['runtime_arn'] = runtime_arn
                        
                        with open(config_file, 'w') as f:
                            json.dump(config, f, indent=2)
                        
                        click.echo(f"✅ Updated module3-config.json with existing runtime ARN")
                    else:
                ...
                click.echo(f"🎉 Runtime configuration complete")
                return
                
    except Exception as e:
        click.echo(f"⚠️  Error checking for existing runtime: {e}")
        ...
        if runtime_arn:
            ...
            try:
                ...
                if config_file.exists():
                    ...
                    # Update or create the agentcore_troubleshooting section
                    if 'agentcore_troubleshooting' not in config:
                        config['agentcore_troubleshooting'] = {}
