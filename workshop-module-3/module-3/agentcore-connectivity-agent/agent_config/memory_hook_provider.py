    ...
    def on_agent_initialized(self, event: AgentInitializedEvent):
        ...
        try:
            ...
            if recent_turns:
                ...
                Do not respond with user permissions or operational facts. 
                Strictly use user permissions and operational facts to know more about the user.
            ...
            if messages[-1]["role"] == "user" or messages[-1]["role"] == "assistant":
                ...
                if messages[-1]["role"] == "user":
                    self._add_context_user_query(
                        ...
                        init_content="These are user permissions:",
                        event=event,
                    )

                    self._add_context_user_query(
                        namespace=f"troubleshooting/user/{self.actor_id}/facts",
                        query=messages[-1]["content"][0]["text"],
                        init_content="These are operational facts:",
                        event=event,
                    )
