...
def main(agent_name: str, prompt: str, interactive: bool):
    ...
    # Get agent ARN from config or fetch from AWS
    agent_arn = runtime_config["agents"][agent_name]["bedrock_agentcore"]["agent_arn"]
    
    # Check if ARN is from current account, if not, fetch from AWS
    import boto3
    try:
        sts = boto3.client('sts')
        current_account = sts.get_caller_identity()['Account']
        
        if current_account not in agent_arn:
            print(f"⚠️  Config has ARN from different account, fetching from current account...")
            agentcore_client = boto3.client('bedrock-agentcore-control', region_name=get_aws_region())
            ...
            for runtime in runtimes.get('agentRuntimes', []):
                if runtime['agentRuntimeName'] == agent_name:
                    ...
                    print(f"✅ Found agent in current account: {agent_arn}")
                    break
            else:
                print(f"❌ Agent '{agent_name}' not found in current account")
                sys.exit(1)
    except Exception as e:
