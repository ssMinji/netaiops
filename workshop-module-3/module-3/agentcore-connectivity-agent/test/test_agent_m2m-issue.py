                ...
                if response.status == 200:
                    ...
                    if tickets:
                        ...
                        for ticket in sorted(tickets, key=lambda x: x.get('created_at', ''), reverse=True):
                            if "ExampleCorp - Connectivity issue found for Imaging Platform - Triaging using A2A" in ticket.get('subject', ''):
                                target_ticket = ticket
                                break
                        
                        if not target_ticket:
                            print("⚠ No ticket found with subject 'ExampleCorp - Connectivity issue found for Imaging Platform - Triaging using A2A'")
    ...
    while True:
        try:
            ...
            if user_input.lower() in ['quit', 'exit']:
                ...
                # Add correspondence to support ticket after session ends
