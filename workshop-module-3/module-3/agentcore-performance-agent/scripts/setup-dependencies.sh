...
# NetOps AgentCore Dependencies Setup Script
# Based on proven working reference implementation

set -e

echo "🔧 NetOps Agent Dependencies Setup"
echo "==================================="
...
setup_python_environment() {
    ...
    # Install bedrock-agentcore with proper Python version (idempotent check)
    ...
    if python3 -c "import bedrock_agentcore" 2>/dev/null; then
        ...
        if pip install "bedrock-agentcore>=0.1.1" --quiet; then
            echo -e "${GREEN}   ✅ bedrock-agentcore SDK installed (bedrock-agentcore>=0.1.1)${NC}"
        else
    ...
    if [[ -f "$REQUIREMENTS_FILE" ]]; then
        ...
        while IFS= read -r line; do
            ...
            # Explicitly install bedrock-agentcore-starter-toolkit
            if [[ "$line" == "bedrock-agentcore-starter-toolkit" ]]; then
                ...
                if pip install "$line" --quiet 2>/dev/null; then
                    ...
                    echo -e "${RED}     ❌ CRITICAL ERROR: Failed to install bedrock-agentcore-starter-toolkit${NC}"
                    exit 1
                fi
            fi
    ...
    # Install essential dependencies (with idempotent checks)
    echo "   📦 Checking essential dependencies..."
    
    # Define essential dependencies with their import names
...
main() {
    ...
    echo "   3. Create gateway: python3 scripts/agentcore_gateway.py create --name netops-gateway"
    echo "   4. Deploy runtime: python3 scripts/agentcore_agent_runtime.py"
    echo ""
    echo -e "${GREEN}✨ Setup complete! You can now run the NetOps AgentCore deployment scripts.${NC}"
}
