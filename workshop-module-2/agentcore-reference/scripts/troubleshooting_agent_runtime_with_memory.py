...
def create(agent_name: str):
    ...
    try:
        ...
        click.echo("⚙️  Step 1/2: Setting up configuration with memory integration...")
        
        # Sanitize external inputs to prevent command injection
        safe_role_arn = shlex.quote(role_arn)
        safe_name = shlex.quote(agent_name)
        ...
        click.echo("🔧 Configuring with OAuth (same as module-1)...")
        click.echo(f"   🔐 OAuth Discovery URL: {oauth_discovery_url}")  
        click.echo(f"   🆔 OAuth Client ID: {oauth_client_id}")
        ...
        click.echo("✅ Memory-enhanced runtime configured successfully")
        if result.stdout:
        ...
        click.echo("🧠 Memory integration enabled via existing SSM parameter")
        
        # Launch runtime with auto-update (handles existing agent)
        click.echo("⚙️  Step 2/2: Launching Memory-Enhanced AgentCore runtime...")
